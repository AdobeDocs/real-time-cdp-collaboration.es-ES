...
Para empezar a agregar tu conexión de datos, selecciona **[!UICONTROL Agregar una nueva conexión de datos]** y luego selecciona **[!UICONTROL Siguiente]**.
...
* **Adobe Experience Platform**: seleccione esta opción para traer sus audiencias desde Adobe Experience Platform.
* **Archivo CSV**: cargue un archivo CSV que contenga los datos de su audiencia para incorporar datos de forma rápida y directa. Consulte la guía [Cargar archivo CSV para el abastecimiento de audiencias](./upload-csv-audience-sourcing.md) para empezar.
...
* **Snowflake** (versión futura): Use su almacén de datos de Snowflake para extraer datos de audiencias sin problemas.
* **Google Cloud Platform** (versión futura): conéctese a su Google Cloud Storage para obtener datos de audiencia directamente de los bloques de GCS.
...
![El espacio de trabajo Agregar audiencias con una zona protegida seleccionada.](/help/assets/setup/add-manage-audiences/select-sandbox.png){zoomable="yes"}
...
* Si la casilla de verificación está ***habilitada***, se excluirán los datos etiquetados en Experience Platform como se ha descrito anteriormente y **no** se incluirán en Collaboration.
...
![El espacio de trabajo Agregar audiencias en la sección Directiva de administración y acciones de cumplimiento.](/help/assets/setup/add-manage-audiences/data-collaboration-consent.png){zoomable="yes"}
...
![El cuadro de diálogo Directiva de gobernanza y acciones de aplicación con la casilla de verificación y la opción Aceptar resaltadas.](/help/assets/setup/add-manage-audiences/data-collaboration-consent-confirmation.png){zoomable="yes"}
...
![El espacio de trabajo Agregar audiencias con la opción de asignar campos de origen a campos de destino.](/help/assets/setup/add-manage-audiences/add-map-fields.png){zoomable="yes"}
...
**[!UICONTROL Los campos de Source]** son áreas de nombres y atributos de identidad de Experience Platform. Estas incluyen áreas de nombres de identidad [estándar](https://experienceleague.adobe.com/docs/experience-platform/identity/features/namespaces.html?lang=es#standard){target="_blank"} y [personalizada](https://experienceleague.adobe.com/docs/experience-platform/identity/features/namespaces.html?lang=es#create-namespaces){target="_blank"}. También incluyen atributos de perfil que están presentes en el [esquema de unión](https://experienceleague.adobe.com/docs/experience-platform/profile/union-schemas/union-schema.html?lang=es){target="_blank"} y pertenecen a la clase de perfil XDM individual.
...
Utilice la opción **[!UICONTROL Aplicar transformación]** cuando vaya a obtener *campos sin hash* en campos con hash. Collaboration aplicará el hash y transformará los campos. El algoritmo hash utilizado por Adobe es SHA256.
...
Para gestionar el abastecimiento de un campo sin hash a un campo de destino con hash, utilice la opción **[!UICONTROL Aplicar transformación]**. Por ejemplo, para agregar un segundo campo de correo electrónico, seleccione la opción **[!UICONTROL Agregar campo]** a para agregar una fila nueva y, a continuación, seleccione **[!UICONTROL Correo electrónico con hash]** para el campo de destino. Seleccione un campo de origen de correo electrónico sin hash y, a continuación, seleccione **[!UICONTROL Aplicar transformación]**.
...
Siga agregando pares de asignación para cada campo de destino. Si no desea utilizar una clave de coincidencia, puede eliminarla mediante el icono Eliminar (![Eliminar icono](/help/assets/icons/delete.png)) que hay junto al campo. Si se elimina la clave de coincidencia, no podrá utilizarla al obtener audiencias de la conexión.

![Espacio de trabajo Agregar audiencias con la opción Eliminar resaltada junto a un campo de destino.](/help/assets/setup/add-manage-audiences/remove-target-field.png){zoomable="yes"}

Cuando haya terminado de asignar campos, seleccione **[!UICONTROL Siguiente]** para continuar.

![Espacio de trabajo Agregar audiencias con los campos asignados rellenados y la opción Siguiente resaltada.](/help/assets/setup/add-manage-audiences/confirm-field-mapping.png){zoomable="yes"}
...
![Agregar espacio de trabajo de audiencia con las opciones de programación mostradas.](/help/assets/setup/add-manage-audiences/audience-scheduling.png){zoomable="yes"}
...
A continuación, seleccione el **[!UICONTROL intervalo de fechas]**. La fecha de inicio es la fecha en la que la audiencia empezará a rellenarse con perfiles y la fecha de finalización es cuando la audiencia dejará de actualizarse.
...
>[!IMPORTANT]
>
>Después de la fecha de finalización en el intervalo de fechas, todas las audiencias procedentes de esta conexión de datos dejarán de actualizarse. Para renovar la conexión, siga la guía [administrar conexión de datos](/help/guide/setup/manage-data-connection.md).
...
Después de obtener audiencias, el espacio de trabajo **[!UICONTROL Mis audiencias]** muestra todas las audiencias que provienen actualmente de Collaboration.

![El espacio de trabajo Mis audiencias muestra todas las audiencias de origen.](/help/assets/setup/add-manage-audiences/audiences-workspace.png)
...
| **[!UICONTROL Acceso a la conexión]** | Defines whether the audience is private or public. Public audiences are discoverable in overlap reports and can be activated within a project. |
| **[!UICONTROL Creado]** | Indicates when the audience was initially sourced into Collaboration. |
...
To perform quick actions on an audience, select the ellipsis **...** next to the audience name. Las opciones disponibles son las siguientes:
...
| Elemento | Descripción |
|----------|---------|
| **[!UICONTROL Estado]** | Indicates if the audience is active and can be used in projects. |
...
Para facilitar la organización, el filtrado y la recuperación de audiencias, puede etiquetar sus audiencias. Puede etiquetar una audiencia con varias categorías y, a continuación, utilizar estas etiquetas de categoría para filtrar las audiencias que desee en el área de producto [descubrir](/help/guide/collaborate/discover.md) al ejecutar informes de superposición de audiencias.
...
![Se muestra el cuadro de diálogo Acceso a la conexión con las opciones disponibles.](/help/assets/setup/add-manage-audiences/audience-details-connection-access-dialog.png){zoomable="yes"}
...
>[!NOTE]
>
>Si el colaborador tiene todas las audiencias definidas como privadas, la sección **[!UICONTROL Audiencias relevantes]** de un proyecto en el área de trabajo de **[!UICONTROL Discover]** estará en blanco. Para obtener más información, lea la guía [descubrir](/help/guide/collaborate/discover.md#relevant-audiences).
...
Aparecerá el cuadro de diálogo **[!UICONTROL Visibilidad de metadatos]**, que le permitirá configurar los ajustes de visibilidad de la audiencia. Hay dos ajustes de visibilidad de metadatos que puede configurar para cada audiencia:

**[!UICONTROL Mostrar recuento de identidades]**: esta opción controla si el colaborador puede ver los recuentos de identidades de las audiencias al [ver los informes de superposición en la ficha de detección](/help/guide/collaborate/discover.md#discover-overlaps) dentro de un proyecto.
...
**[!UICONTROL Índice de audiencia]**: cuando se establece en true, los colaboradores pueden ver el [índice de audiencia](/help/guide/collaborate/discover.md#audience-index-score) dentro de un proyecto. Esta funcionalidad solo está disponible cuando tiene tres o más públicos activos.
...
Desde el panel de audiencias, puede editar varias audiencias a la vez. To do this, select the audiences you want to edit by selecting the boxes next to their names. Once you&#39;ve selected the audiences, you can perform actions using the options available in the edit menu.
...
Aparecerá el cuadro de diálogo **[!UICONTROL Categorías]**, que le permitirá configurar las categorías de cada audiencia seleccionada. De forma predeterminada, no se seleccionará ninguna categoría. Para seleccionar una categoría, primero seleccione la categoría principal y luego seleccione las subcategorías que desee incluir. Realice los cambios y, a continuación, seleccione **[!UICONTROL Guardar]**.
